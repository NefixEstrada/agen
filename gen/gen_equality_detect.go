// Vendored from ogen v1.24.0 gen/gen_equality_detect.go.
// Local deltas: g.tstorage is a plain map view (lowering.TStorageView).
...
func (g *Generator) collectEqualitySpecs() {
	...
	names := make([]string, 0, len(g.tstorage))
	for name := range g.tstorage {
		names = append(names, name)
	}
	sort.Strings(names)
	for _, name := range names {
		g.collectFromType(g.tstorage[name], visited)
	}
}
