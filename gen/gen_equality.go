// Vendored from ogen v1.24.0 gen/gen_equality.go.
// Local deltas: header "by agen", file names aas_*, fs is mandatory (no os fallback).
...
func (g *Generator) generateEqualMethod(spec *ir.EqualityMethodSpec, pkgName string, fs FileSystem) error {
	...
	// Write to file
	filename := fmt.Sprintf("aas_%s_equal_gen.go", snakeCase(spec.TypeName))
...
func (g *Generator) generateHashMethod(spec *ir.EqualityMethodSpec, pkgName string, fs FileSystem) error {
	...
	// Write to file
	filename := fmt.Sprintf("aas_%s_hash_gen.go", snakeCase(spec.TypeName))
	return writeFormattedCode(fs, filename, []byte(b.String()))
}
