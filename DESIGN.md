...
|---|---|---|
| redis 0.2.0 (draft) | **M2 — first backend** | Channel `type` (`stream` \| `pubsub`, required) + `maxLen` (exact `XADD ... MAXLEN`, streams only); operation `consumerGroup` (`XREADGROUP`). The version is unpublished, so the fields are read from the `x-redis` specification extension; `bindings.redis` carrying them is a located error pointing there (§6.5). |
...
### 6.5 Redis conventions

The official Redis binding defines real fields as of draft 0.2.0 ([asyncapi/bindings#313](https://github.com/asyncapi/bindings/pull/313)): channel `type` (`stream` | `pubsub`, required) and `maxLen` (exact `XADD ... MAXLEN`, streams only, ≥ 1), operation `consumerGroup` (`XREADGROUP`). The version is not published yet, so agen reads the fields **only** from the `x-redis` specification extension — `bindings.redis` carrying 0.2.0 fields (or the 0.2.0 version marker) is a located parse error pointing at the extension, while fieldless bindings (the published 0.1.0 stub) stay valid and ignored. When the binding ships, `bindings.redis` becomes the primary spelling and `x-redis` a compatibility alias. The spec is the single source of the mapping and the generated code exposes **no mode or group options at all**: every channel must declare its `type` and every streams-mode receive operation its `consumerGroup` — an undeclared value is a located generation error, never a silent default — `pubsub` channels yield pub/sub constructors, and `maxLen` trims published streams. Heterogeneous declarations are located generation errors (the runtime wires a single consumer and publisher). Direct `runtime/redis` users keep the full `ConsumerConfig`/`PublisherConfig` surface, so runtime tuning without generated code is unchanged:
...
- **Mode `streams` (default)** — the durable, at-least-once mapping:
  ...
  - `receive` operation ⇒ `XREADGROUP GROUP <group> <consumer> ... STREAMS <address> >` with the group auto-created (`MKSTREAM`); the group is the operation's `x-redis` consumerGroup, required — the generated code offers no `WithGroup`, direct `runtime/redis` users set `ConsumerConfig.Group`. The binding's groupless `XREAD` reading is not implemented.
...
Extensions: `x-agen-name`, `x-agen-properties`, `x-agen-type`, `x-agen-time-format`, `x-agen-validate` — deliberately mirroring ogen's `x-ogen-*` family for muscle memory — plus `x-redis` (§6.5): the draft redis binding 0.2.0 fields, the spec-level source of the Redis mapping.
...
| agen's Redis conventions diverge from the official Redis binding | the draft 0.2.0 fields are consumed natively via `x-redis` (§6.5); adopting the published `bindings.redis` spelling later is a parser tweak, not a spec break for users |
